...
if __name__ == "__main__":
    ...
    data_dir = opt[
        "data_dir"
    ]  # root directory containing the low-res images you want to super-resolve
    n_lr_images = opt[
        "n_lr_images"
    ]  # number of low-res images as input to the model; must be the same as when the model was trained
    ...
    if "pretrain_network_g" not in opt["path"]:
        ...
        model.load_state_dict(
            state_dict[opt["path"]["param_key_g"]], strict=opt["path"]["strict_load_g"]
        )
    ...
    for i, png in enumerate(pngs):
        ...
        output = np.transpose(output * 255, (1, 2, 0)).astype(
            np.uint8
        )  # transpose to [h, w, 3] to save as image
    ...
    for tile in os.listdir(data_dir):
        ...
        if len(os.listdir(os.path.join(data_dir, tile))) < 256:
            print(
                "Tile ",
                tile,
                " contains less than 256 chunks, cannot stitch. Skipping.",
            )
            continue
